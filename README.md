...
The source code version is download version that not builded, in other word it's the raw code. People usually prefer this version to made sure there isn't anything suspicious because if it's source code they can check it themself. You typically can't do this if it's in executeable format because usually executeable contain binary which make it difficult to check. 

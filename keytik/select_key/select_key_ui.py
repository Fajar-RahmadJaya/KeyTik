...
from PySide6.QtCore import QPoint, Qt  # pylint: disable=E0611
...
class SelectKeyUI:
    ...
    def select_key_top(self, select_key_window, hide_block, context):
        ...
        filter_button.clicked.connect(lambda: self.show_filter_popup(search_entry, filter_popup))
        ...
        if context != "remap":
            ...
            self.search_unicode_checkbox.setToolTip("Unicode only supported on remap key.")
        else:
    ...
    def search_with_unicode(self, search_text):
        ...
        # Search unicode
        try:
            minimim_search_text = 3
            if search_text.isalpha() and len(search_text) > minimim_search_text:
    ...
    def click_checkbox(self, item, select_key_entry):
        ...
        if item.parent() is None:
            ...
            elif key_tuple in self.checked_keys_list:
                self.checked_keys_list.remove(key_tuple)

            select_key_entry.setText(" + ".join([child for _, child in self.checked_keys_list]))
